...
interface DivorciadoProps {
    onRequisitosChange?: (requisitos: RequisitoDivorciado[]) => void;

}

const REQUISITOS_DIVORCIADO: RequisitoDivorciado[] = [
    {
        id: '1',
        titulo: 'Partida de Matrimonio con la respectiva acta de divorcio.',
    ...
    {
        id: '2',
        titulo: 'Copia Actulizada del DNI con el estado civil de divorciado(a).',
        descripcion: 'Documento Nacional de Identidad vigente(original y copia).',
        obligatorio: true,
        completado: false
    }
];
...
const Divorciado = ({
    ...
    const [requiDivorciado, setRequisitos] = useState<RequisitoDivorciado[]>(REQUISITOS_DIVORCIADO);
    const [isDragging, setIsDragging] = useState(false);

    // Notificar cambios en requisitos
    useEffect(() => {
        if (onRequisitosChange) {
            onRequisitosChange(requiDivorciado);
        }
    }, [requiDivorciado, onRequisitosChange]);



    // Calcular progreso
    const calcularProgreso = useCallback(() => {
        const requisitosObligatorios = requiDivorciado.filter(r => r.obligatorio);
        ...
        <div>
            ...
            <div className="space-y-3">
                ...
                <div className="bg-white border border-gray-200 rounded-lg divide-y divide-gray-200">
                    {requiDivorciado.map((requisito, index) => (
                        <div
                            key={requisito.id}
