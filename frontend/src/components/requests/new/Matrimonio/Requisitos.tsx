...
interface ContrayenteCellProps {
    ...
    onCheckboxChange: (id: string | number, idx: number) => void;
                ...
                <div className='grid grid-cols-2 items-start'>
                    ...
                    <div className=''>
                        <div className='group'>
                            ...
                            <p className="absolute rounded-md mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white p-2 shadow">
...
/* Sub-componente toggle para observación */
...
/* ═══════════════════════════════════════════════
   Metadata de condiciones
   ═══════════════════════════════════════════════ */
const CONDICION_META: Record<string, { iconBg: string; iconColor: string; tagClass: string; tagLabel: string }> = {
    GENERAL: { iconBg: '#2563eb', iconColor: '#ffffff', tagClass: 'bg-blue-100 text-blue-800', tagLabel: 'General' },
    DIVORCED: { iconBg: '#ea580c', iconColor: '#ffffff', tagClass: 'bg-orange-100 text-orange-800', tagLabel: 'Divorciado' },
    WIDOWED: { iconBg: '#7c3aed', iconColor: '#ffffff', tagClass: 'bg-purple-100 text-purple-800', tagLabel: 'Viudo' },
};
...
const RequisitosMatrimonio = ({
    ...
    useEffect(() => {
        if (onValidationChange) {
            const isStepValid = progreso.completados > 0 && erroresArchivo.size === 0;
            onValidationChange(isStepValid);
        }
    }, [progreso.completados, erroresArchivo, onValidationChange]);

    // Agrupar requisitos por condición (sin duplicar)
    ...
    // ¿El contrayente necesita esta condición?
                ...
                return (
                    <div key={condicion} className="space-y-3">
                        ...
                        <div className="flex items-center gap-2.5 pb-2 border-b-2 border-gray-200">
                            <div
                                className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm bg-info-content"
                            >
                        ...
                        <div className="overflow-x-auto">
                            <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-200 min-w-180">
                                ...
                                <div className="grid grid-cols-4">
                                    <div className="px-4 py-3 bg-info-content text-white  font-semibold uppercase  items-center gap-2 border-b-2 border-gray-200 col-span-2">
                                        <i className="fas fa-list-check"></i> Requisito
                                    </div>
                                    <div className="px-4 py-3 bg-blue-600 text-white  font-semibold uppercase tracking-wider flex items-center text-xs gap-2 border-b-2 border-gray-200 col-span-1">
                                        <i className="fas fa-mars"></i>
                                        <span className="truncate">
                                            {contrayentes[0]?.nombre || 'Contrayente 1'}
                                        </span>
                                    </div>
                                    <div className="px-4 py-3 bg-pink-600 text-white  font-semibold uppercase tracking-wider flex items-center text-xs gap-2 border-b-2 border-gray-200 col-span-1">
                                        <i className="fas fa-venus"></i>
                                        <span className="truncate">
                                            {contrayentes[1]?.nombre || 'Contrayente 2'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {reqs.map((req, idx) => {
                                    ...
                                    return (
                                        <div key={req.id} className="grid grid-cols-4">
                                            {/* Celda requisito */}
                                            <div className={`p-4 border-b border-l border-gray-300 col-span-2 flex flex-row justify-between ${altBg}`}>
                                                <div>
                                                ...
                                                <div className='flex flex-col justify-between gap-2'>
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            const ambosChecked = requisitosEstados.get(key1) === 1 && requisitosEstados.get(key2) === 1;
                                                            if (!ambosChecked) {
                                            ...
                                            {applies1 ? (
                                                <ContrayenteCell
                                                    reqId={req.id}
                                                    contrayenteIndex={1}
                                                    className={`border-l border-r border-b border-blue-300 bg-blue-100 ${altBg}`}
                                                ...
                                                <div className={`p-4 border-b border-r border-gray-300 flex items-center justify-center ${altBg}`}>
                                            ...
                                            {applies2 ? (
                                                <ContrayenteCell
                                                    reqId={req.id}
                                                    contrayenteIndex={2}
                                                    className={`border-r border-b border-pink-300 bg-pink-100 ${altBg}`}
                                                    requisitosEstados={requisitosEstados}
                                                    archivosRequisitos={archivosRequisitos}
                                                    observacionesMap={observacionesMap}
                                                    erroresArchivo={erroresArchivo}
                                                ...
                                                <div className={`p-4 border-b border-gray-300 flex items-center justify-center ${altBg}`}>
                                                    <span className="text-xs text-gray-300 italic">No aplica</span>
                                                </div>
                                            )}
                                        </div>
